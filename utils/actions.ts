...
export const createProfileAction = async (
  ...
  export const updateProfileImageAction=async(prevState: any,
    formData: FormData):Promise<{message:string}>=>{
      ...
      try {
        ...
        const validatedImage =imageSchema.safeParse({image:file})
        if (!validatedImage.success) {
          const errors = validatedImage.error.errors.map((error) => error.message);
...
export const findExistingReview = async ( userId: string,
  propertyId: string)=>{
...
export const deleteBookingAction=async(prevState: { bookingId: string }): Promise<{ message: string }>=>{
